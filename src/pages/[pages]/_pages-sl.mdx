...
Kako se doda nov stran? Malo smo se že dotaknili na strani "O projektu" kako je strukura strukturiana in kako deluje dinamično usmerjanje ter kako so poimenovane datoteke.

Za omogočanje lokalizacije sta dve možniosti:

1. Uporabiti lahko eno samo datoteko z dinamičnim parametrom, npr. `[...pages].astro` (kot je prikazano tukaj). Strani ki uporabljajo ta struktiso so [...index].astro, [...blog].astro ter [...contact].astro
...
2. Uporabiti lahko mapo za vsako stran, npr. `[about]/[...index].astro` (kot je prikazano na strani "O projektu").

Strani ki uporabljajo ta struktiso so "about"","blog", "pagination", "pages" ter "services"
...
| `[...pages].astro`         | `/pages`     | `/sl/strani`                 | Splošne strani |
...
### 3. Alternativne strukture strani
...
> **Pomembno**: Ključi se morajo ujemati z imeni datotek v `pages/` mapi!

### 2. `src/i18n/ui.ts` - Konfiguracija jezikov

Ta datoteka vsebuje ključne konfiguracije za jezikovno podporo:

-   **`localeModules`** - funkcija, ki glede na izbrani jezik pridobi vse JSON datoteke z besedili za ta jezik
...
const localizedUrl = translatePath("/about"); // "/o-projektu" za slovenščino
```
...
const navigationData = [
    {
        key: "menu.list.home", // Ključ za prevod
        url: "/", // Angleška pot (privzeta)
        children: [],
    },
    {
        key: "menu.list.service",
        url: "/services",
        children: [
            {
...
```json
{
    "nik-klemenc": {
        ...
        "position": {
            ...
            "sl": "Razvijalec celotnega sklada"
        }
    }
}
```
...
Projekt vključuje primer paginacije v `[pagination]/[...slug].astro`, ki je implementiran brez uporabe Astro-jeve privzete `paginate()` funkcije.

```javascript
// Ročno implementirana paginacija
export function getStaticPaths() {
    // Logika za generiranje strani paginacije
    // Podpira lokalizirane URL-je
}
```

> **Opomba**: Projekt vključuje tudi primer ročno implementirane paginacije v `[pagination]/[...slug].astro`.
>
> Paginacija ni implementirana z uporabo privzete Astro `paginate()` funkcije, ampak je logika napisana od začetka. To je bilo potrebno zaradi tehničnih izzivov pri integraciji s sistemom i18n. Če uspe komu integrirati privzeto `paginate()` funkcijo z lokaliziranimi URL-ji, bom hvaležen za pull request.
