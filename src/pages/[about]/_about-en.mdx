...
| `[...pages].astro`         | `/pages`     | `/sl/strani`                 | General pages |
...
> **Important**: Keys must match file names in the `pages/` directory!

### 2. `src/i18n/ui.ts` - Language Configuration

This file contains key configurations for language support:

-   **`localeModules`** - function that retrieves all JSON files with texts for the selected language
...
const localizedUrl = translatePath("/about"); // "/o-projektu" for Slovenian
```
...
const navigationData = [
    {
        key: "menu.list.home", // Translation key
        url: "/", // English path (default)
        children: [],
    },
    {
...
The project includes an example of pagination in `[pagination]/[...slug].astro`, implemented without using Astro's default `paginate()` function.

```javascript
// Manually implemented pagination
export function getStaticPaths() {
    // Logic for generating pagination pages
    // Supports localized URLs
}
```

> **Note**: The project also includes an example of manually implemented pagination in `[pagination]/[...slug].astro`.
>
> Pagination is not implemented using the default Astro `paginate()` function, but the logic is written from scratch. This was necessary due to technical challenges integrating with the i18n system. If someone manages to integrate the default `paginate()` function with localized URLs, we would appreciate a pull request.
